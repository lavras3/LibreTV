...
try {
    ...
    // 转换静态页面内容
    const htmlConverter = OpenCC.HTMLConverter({ from: 'cn', to: 'tw' });
...
function initializePageContent() {
    ...
    // 设置页面标题
    const convertedTitle = convertToTraditional(currentVideoTitle);
    document.title = convertedTitle + ' - LibreTV播放器';
    document.getElementById('videoTitle').textContent = convertedTitle;
...
function renderResourceInfoBar() {
    ...
    container.innerHTML = `
      <div class="resource-info-bar-left flex">
        <span>${convertToTraditional(resourceName)}</span>
...
async function showSwitchResourceModal() {
    ...
    modalTitle.innerHTML = `<span class="break-words">${convertToTraditional(currentVideoTitle)}</span>`;
    ...
    for (const [sourceKey, result] of sortedResults) {
        ...
        const sourceName = convertToTraditional(resourceOptions.find(opt => opt.key === sourceKey)?.name || '未知资源');
